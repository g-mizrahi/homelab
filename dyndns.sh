...
#
# Account Variables
# The email is the one used to login on Clouflare
...
# Log the error and exit the program
if [[ $? != "0" ]]; then
  echo {\"date\":\"$(date +%Y-%m-%dT%H:%M:%S)\", \"level\":\"error\", \"message\":\"Failed to get the server IP from ipify\"} >> $err_log_path
  exit 1
else
  # Log the public IP
  # To keep track of how often the public IP of the server changes
  echo {\"date\":\"$(date +%Y-%m-%dT%H:%M:%S)\", \"ip\":\"$server_ip\"} >> $ip_log_path
fi
...
if [[ "$dns_ip" != "$server_ip" ]]; then
  ...
  if [[ "$success" != "true" ]]; then
    echo {\"date\":\"$(date +%Y-%m-%dT%H:%M:%S)\", \"level\":\"error\", \"message\":\"$new_dns\"} >> $err_log_path
    exit 1
  elif [[ "$success" == "true" ]]; then
    echo {\"date\":\"$(date +%Y-%m-%dT%H:%M:%S)\", \"level\":\"debug\", \"message\":\"$new_dns\"} >> $err_log_path
    exit 0
  fi
fi
